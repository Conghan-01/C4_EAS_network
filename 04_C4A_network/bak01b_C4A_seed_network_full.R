...
expr_file <- "/gpfs/hpc/home/chenchao/hanc/project/C4_CNB2025/02_C4_network/networkv2/prenatal.logTMM.ComBat.txt"
cov_file <- "/gpfs/hpc/home/chenchao/hanc/project/C4_CNB2025/02_C4_network/networkv2/prenatalcovarites.expressPC_knownfornetwork"
...
datCov<- as.data.frame(datCov)
# selection 1. covs_to_regress <- as.data.frame(datCov %>% dplyr::select(sex, RIN, GW))
# selection 2. 
...
write.csv(pos_net, "C4A_Positive_Network_pre.csv", row.names = FALSE)
write.csv(neg_net, "C4A_Negative_Network_pre.csv", row.names = FALSE)
...
run_enrichment <- function(gene_list, background, prefix) {
  ...
  if (!is.null(ego) && nrow(ego@result) > 0) {
    write.table(ego@result, paste0("C4A_GO_Result_", prefix, ".txt"), 
...
if(!is.null(pos_go)) {
  ...
  if(!is.null(p1)) {
    print(p1)
    ggsave("C4A_GO_prenatal_Positive.png", p1, width = 7, height = 6, dpi = 400)
...
if(!is.null(neg_go)) {
 ...
 #  #83bcd7 postantal  ; #e89a92 #prenatal
  if(!is.null(p2)) {
    print(p2)
    ggsave("C4A_GO_Prenatal_Negative.png", p2, width = 6, height = 5, dpi = 400)
    #ggsave("C4A_GO_prenatal_Negative.pdf", p2, width = 7, height = 6)
  }
}

# ==============================================================================
# 2.  postnatal: C4A seed co-expression network
# ==============================================================================

#expr_file <- "/gpfs/hpc/home/chenchao/hanc/project/2023project_develop_eQTL/04_analysis/05deconvolution/bulk_data/postnatal.logTMM.ComBat.txt"
#cov_file <- "/gpfs/hpc/home/chenchao/hanc/project/2023project_develop_eQTL/04_analysis/05deconvolution/bulk_data/postnatalcovariatesToUse.PCAforQTLfromINT.txt"
#prenatal_exp<- read.table("/gpfs/hpc/home/chenchao/hanc/project/2023project_develop_eQTL/04_analysis/05deconvolution/bulk_data/stage1.logTMM.ComBat.txt")

expr_file <- "/gpfs/hpc/home/chenchao/hanc/project/C4_CNB2025/02_C4_network/networkv2/postnatal.logTMM.ComBat.txt"
cov_file <- "/gpfs/hpc/home/chenchao/hanc/project/C4_CNB2025/02_C4_network/networkv2/postnatalcovarites.expressPC_knownfornetwork"
...
#datExpr<- datExpr[prenatal_genes,]
## Or: datExpr<-  read.table(expr_file, header = TRUE, row.names = 1, check.names = FALSE, stringsAsFactors = FALSE)
datCov <- fread(cov_file, header = TRUE, sep = "\t", check.names = FALSE)
# datCov <- fread(cov_file, header = TRUE, sep = "\t", check.names = FALSE)
...
write.csv(pos_net, "C4A_Positive_Network_post.csv", row.names = FALSE)
write.csv(neg_net, "C4A_Negative_Network_post.csv", row.names = FALSE)
